...
exports.login = (req, res) => {
  ...
  User.login(user, (err, data) => {
    if (err) {
      res.status(401).json({
        message:
          err.msg || "Some error occured while logging the User."
      });
    } else {
...
exports.forgot = (req, res) => {
  ...
  User.forgot(user, (err, data) => {
    if (err) {
      res.status(403).json({
        message:
          err.msg || "Some error occured while changing user's password."
      });
    }
    else {
      let payload = { email: data.email };
  ...
  jwt.verify(req.params.token, process.env.ACCESS_TOKEN_SECRET, (err, decoded) => {
    ...
    User.confirm(decoded.email, (err, data) => {
      if (err) {
        res.status(403).json({
          message:
            err.msg || "Some error occured while confirming the User."
        });
      }
    });
  });
...
exports.findOne = (req, res) => {
  ...
  User.findById(username, password, (err, data) => {
  console.log(req.body.username);
  console.log(req.body.password);
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: `Not found User with password " + ${req.body.password} + " and username " + ${req.body.username}.`
        });
      } else {
        res.status(500).send({
          message: "Error retrieving User with password " + req.body.password + " and username " + req.body.username
        });
      }
    } else res.send(data);
  });
